...
function updateorderStatus(id, ths, type) {
  ...
  if (!(prevStatus != "Ordered" && status == "Ordered")) {
    ...
    $.simplyToast(
      "Can't move your status to orderd as it is already inprogress",
      "warning"
    );
    ...
    success: function (response) {
      ...
      if (response == "") {
        ...
        $.each(response, function (index, value) {
          if (index % 2 == 0) {
            $("#divuseraddresseven").append(
              "<div class='panel panel-default' style='width: 18rem;padding-bottom: 25px;'><div class='panel-body' style='border-style: solid;border-color: #dee2e6;'><address class='card-text' style='color:black;font-style:Arial,sans-serif;text-align: center;'>Name: " +
                response[index].name +
                "<br> Mobile: " +
                ...
                "<br> Pincode: " +
                response[index].pincode +
                "<br> Landmark: " +
                response[index].landmark +
                "<br> Line1: " +
                response[index].line1 +
                "</address></div></div>"
            );
          } else {
            $("#divuseraddressodd").append(
              "<div class='panel panel-default' style='width: 18rem;padding-bottom: 25px;'><div class='panel-body' style='border-style: solid;border-color: #dee2e6;'><address class='card-text' style='color:black;font-style:Arial,sans-serif;text-align: center;'>Name: " +
                response[index].name +
                "<br> Mobile: " +
                response[index].mobile +
        ...
        $.each(response, function (index, value) {
          ...
          $("#tblorderedproducts").append(
            "<tr><td>" +
              ...
              "</td></td><td>" +
              response[index].original +
              "</td><td class='price'>" +
              response[index].selling +
              "</td></tr>"
          );
        });
